...
rushdata_2017[2,] <- toupper(paste0(rushdata_2017[2,], rushdata_2017[3,]))

rushdata_2017$X__1 <- as.character(rushdata_2017$X__1)
rushdata_2017$X__1[2] <- "Location"
...
rushdata_2016 <- 
  rushdata_2016[which(!rushdata_2016$X__1 %in%
                       c("Combined", "plot_ID", "Type", "Site", "Year", 
                         ...
                         "Total sedge cover", "Rush cover", "Grass cover", 
...
#reorganise
startcols <- c("year", "Location", "Loaction", 
               "Replicate.number", "Replicate..A.B.C.", 
               "Treatment.Plot", "Treatment.Plot..1.8.", 
               "Site..H.L.V.", "Type..M.P.",
               "Actual.Quadrat.Number", "Quadrat..1.100.", 
               "Total.mosses", "Bryophyte.cover", "NA.", "NA..1")

rushdata_t <- select(rushdata_t, one_of(startcols),everything())

#have a look where there are missing values
VIM::matrixplot(rushdata_t[1:15], 
                labels = substr(names(rushdata_t[1:15]), 1, 5))
